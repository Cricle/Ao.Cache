...
{{
 ...
 {GetVisibility(declare.DeclaredAccessibility)} class {name}{proxyEndName} : {declare}
    {{    
        ...
        {string.Join("\n        ", canProxys.Select(x=>$@" private static readonly System.Reflection.MethodInfo {GetMethodInfoName(methods.IndexOf(x),x,syntaxContext.SemanticModel)};"))}
            ...
            if (proxy)
            {
                ...
                else if(method.ParameterList.Parameters.Count!=0)
                {
                    ...
                    else
                    {
                        ...
                        keyGen += $"            var interpolatedStringHandler=new System.Runtime.CompilerServices.DefaultInterpolatedStringHandler({method.ParameterList.Parameters.Count * 2 - 1},0);\n";
